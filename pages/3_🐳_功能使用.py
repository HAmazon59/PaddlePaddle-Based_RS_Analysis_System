...
def draw_counts_data(select):
    if len(select) > 1:
        ...
        if "playground" in add_radio:
            exec(
                "col%s.metric('playground', '数量:%s', '置信度>%s')" % (sums, playground["count"], playground["confidence"]))
            sums = sums + 1
...
if tag == choice[0]:
    ...
    if uploaded_file1 is not None:
        ...
        with st.spinner("请稍候......"):
            predictor = Predictor(Model_address_TargetExtraction, use_gpu=st.session_state.computer_type)
            ...
            with st.expander("提取结果"):
                st.image(image, channels="RGB", caption='提取结果可视化')
        ...
        with st.spinner('可视化渲染......'):
            with st.expander("提取结果可视化", expanded=True):
                ...
                st.image(out, channels="BGR", caption='道路提取可视化')
...
if tag == choice[1]:
    ...
    if uploaded_file1 is not None and uploaded_file2 is not None:
        ...
        with st.spinner("请稍候......"):
            predictor = Predictor(Model_address_ChangeDetection, use_gpu=st.session_state.computer_type)
        ...
        with st.spinner('可视化渲染......'):
            with st.expander("检测结果可视化", expanded=True):
                ...
                new_transparent, change_radio = black2transparent(white_img,
                                                                  [r, g, b, alpha2])  # 获取被提取的道路像素，转为蓝色 --》可增加颜色选择模块
                cv2.imwrite('cache/new_transparent.png', new_transparent)
                new_transparent = cv2.imread('cache/new_transparent.png')
                ...
                if choose == "时相图A":
                    if uploaded_file1 is not None and uploaded_file2 is not None:
                        ...
                        st.image(out, channels="BGR", caption='变化检测可视化')
...
if tag == choice[2]:
    ...
    if uploaded_file1 is not None and add_radio:
        ...
        with st.spinner("请稍候......"):
            ...
            if "playground" in add_radio:
                flag = False
                predictor = Predictor(Model_address_TargetDetection+"/playground", use_gpu=st.session_state.computer_type)  # 加载模型
            ...
            if "overpass" in add_radio:
                predictor = Predictor(Model_address_TargetDetection+"/overpass", use_gpu=st.session_state.computer_type)
                resu = predictor.predict(mb)
            ...
            if "aircraft" in add_radio:
                predictor = Predictor(Model_address_TargetDetection+"/aircraft", use_gpu=st.session_state.computer_type)
                resu = predictor.predict(mb)
            ...
            if not flag:
                with st.expander("目标检测结果", expanded=True):
                    st.image(vis, channels="BGR", caption='目标检测可视化')
                    st.subheader("检测结果：")
    ...
    if uploaded_file1 is not None:
        with st.spinner("请稍候......"):
            with st.expander('分类结果', expanded=True):
                predictor = Predictor(Model_address_TerrainClassification, use_gpu=st.session_state.computer_type)
